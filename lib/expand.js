 ...
 *
 * @param {string} goal
 * @param {object} ctx { cwd, model, effort, apiKey, maxDepth, maxLeaves, repoSummary }
...
async function expandNode(raw, { prefix, depth, ancestorDecisions, ctx, state }) {
  ...
  if (!depthOk || !leavesOk) {
    ...
    if (!leavesOk) {
      throw new Error(
        `Cannot coerce subtree "${globalId}" to a leaf: max-leaves ${ctx.maxLeaves} already reached. ` +
          'Raise --max-leaves or ask the planner for fewer children.',
      );
    }
...
function claimLeaf(state, ctx, globalId) {
  if (state.leaves >= ctx.maxLeaves) {
    throw new Error(
      `Leaf budget exceeded while accepting "${globalId}" ` +
        `(max-leaves ${ctx.maxLeaves}). Raise --max-leaves or ask the planner for fewer children.`,
    );
  }
  state.leaves += 1;
}

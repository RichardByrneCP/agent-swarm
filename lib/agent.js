 ...
 * @param {string} [params.effort] reasoning effort (passed as model param `effort` when set)
 * @param {Array<{ id: string, value: string }>} [params.modelParams] extra model params
...
export async function runAgent({
  ...
  try {
    ...
    agent = await Agent.create({
      apiKey,
      model: buildModelSelection(model, effort, modelParams),
...
/**
 * Build an SDK ModelSelection from id + optional effort / params.
 * `effort` is folded into `params` as `{ id: "effort", value }` when set;
 * explicit modelParams win over effort for the same id.
 */
export function buildModelSelection(modelId, effort, modelParams) {
  const params = [];
  if (effort) params.push({ id: 'effort', value: String(effort) });
  if (Array.isArray(modelParams)) {
    for (const p of modelParams) {
      if (!p?.id) continue;
      const idx = params.findIndex((x) => x.id === p.id);
      const entry = { id: p.id, value: String(p.value) };
      if (idx >= 0) params[idx] = entry;
      else params.push(entry);
    }
  }
  return params.length ? { id: modelId, params } : { id: modelId };
}

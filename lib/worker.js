...
export async function runWorker({ leaf, cwd, model, apiKey, shared, reviewNotes }) {
  ...
  // Workers may finish with tool-only output and no prose; allow empty text.
  const res = await runAgent({ prompt, model, cwd, apiKey, role: 'worker', allowEmptyText: true });
  return {
    ok: res.ok,
    status: res.status,
    error: res.error,
...
When finished, end your response with a markdown heading exactly equal to "${NOTES_MARKER}" on its own line, followed by 0-5 short bullet points capturing anything surprising or useful for teammates (conventions, gotchas, decisions you had to infer). If nothing is worth noting, write that heading followed by a single line containing only "none".`;
}

function extractNotes(text) {
  if (typeof text !== 'string') return '';
  // Prefer the structured heading form; fall back to legacy marker for older runs.
  const markers = [NOTES_MARKER, 'FIELD GUIDE NOTES'];
  let idx = -1;
  let marker = NOTES_MARKER;
  for (const m of markers) {
    const at = text.lastIndexOf(m);
    if (at > idx) {
      idx = at;
      marker = m;
    }
  ...
  if (/^none$/i.test(firstLine)) return '';
  return after.trim();
}
